...
/*   Updated: 2022/02/09 18:11:37 by jbosquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	fill_struct(t_minishell *minishell, char ***cmds, t_list *garbage)
{
	...
	size_of_3array(cmds) + 1, t_list *garbage);
	...
	parse_new_line(t_minishell *minishell, char *new_line)
{
	...
	if (new_line == NULL)
		return ;
	line_cmds = ft_split_with_quotes(new_line, '|', minishel->garbage);
	if (!line_cmds)
		return ;
	cmds_split = ft_calloc(sizeof(char **), size_of_array(line_cmds) + 1, minishel->garbage);
	if (cmds_split == NULL)
		free_parse(cmds_split, line_cmds, 0, size_of_array(line_cmds));
	while (line_cmds[i])
	{
		cmds_split[i] = ft_split_with_quotes(line_cmds[i], ' ', minishel->garbage);
